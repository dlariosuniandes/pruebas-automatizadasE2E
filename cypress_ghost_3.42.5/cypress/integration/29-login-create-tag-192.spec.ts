...
describe("Should login and create a tag with name successfully", () => {
  ...
  before(() => {
    ...
    cy.fixture('tag-data-apriori.json').then((data)  => {
      let varTag = data[Math.floor(Math.random() * data.length)];
      tagTitle = varTag.tag_192;
  ...
  it("tag with random title dont should be available on post list", () => {
    cy.contains("Are you sure you want to leave this page?").should("exist").wait(5000);
  });
});
